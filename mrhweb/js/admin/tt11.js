initTypes = function(){
		...
		(function(w){
			...
			window._postDataJson({
	    		...
	    		success : function(data){
	    			...
	    			tr_tdList(downlList);
	    			var $pag = $("#footable-res2 .pagination");
	...
	tr_tdList = function(d){
			...
			(function(w){
				...
				for(var i = 0 ,length = d.length; i < length; i ++){
					...
					var $td5 = $("<td>"+d[i].author+"</td>");
					var $td6 = $("<td></td>");
					var $span1 = $('<span class="status-metro status-active"><a href="'+window._BasePath+'/governor/tutorial/edit.do?id='+d[i].id+'">编辑</a></span>');
					var $span2 = $("<span class='status-metro status-suspended'>删除</span>")
									.attr({"onclick":"deleteTutor('"+d[i].id+"','"+d[i].type+"','"+d[i].none+"')"});
					...
					$td6.append($(labelNo));
					$tr.append($td0).append($td1).append($td2).append($td3).append($td5).append($td6);
					$tbody.append($tr);
				}
				$newF.footable();
	...
	stickThis = function(id,obj){
		...
		window._postDataJson({
			showMessage : false,
			url : window._BasePath + "/governor/tutorial/stickOne.json",
	...
	updateStick = function(id,obj){
		$.ajax({
			url : window._BasePath + "/governor/tutorial/updateStick.json",
			data : {id:id},
			type : 'post',
			success : function(data){
				if(data=="true"){
	...
	deleteTutor = function(id,type,flag){
		if(type == 1 && flag=="true"){
			alert("非空文件夹不可删除");	
		}else{
