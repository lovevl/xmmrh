dowlTypesBorder = function() {
	var QSandAnimationStart = true;
	// get the action filter option item on page load
	var jQueryfilterType = "all";
	...
	jQueryholder.find('li').each(function(i){
		...
		borderBool = (($('body').width()>=748 && i!=0 && i!=1 && i!=2 && i!=3 ) || ($('body').width()<748 && i!=0 && i!=1));
		var clearBool = ($('body').width()<748);
...
function  servicesInit(QSandInit, jQueryholder, jQuerydata, jQueryfilterType, jQueryfilterType) {
	...
	jQueryfilteredData.each(function(i){
			...
			var borderBool = false;
			borderBool = (($('body').width()>=752 && i!=0 && i!=1 && i!=2 && i!=3) || ($('body').width()<752 && i!=0 && i!=1));
			if(borderBool){
