...
$.fn.conRollover = function(a) {
	...
	$(this).each(function(){
		...
		var text = "";
		switch($(this).attr('data-description')){
			case "ckgd" : text = "查看更多";break;
			case "ckxq" : text = "查看详情";break;
			case "djdt" : text = "点击大图";break;
			case "djxz" : text = "点击下载";break;
			default : text = "查看更多";break;
		}
		
		if(type != 'content') $(this).append('\n<div class="image_roll_zoom"><div style="padding:'+($(this).height()/2 - ($(this).height()>=150 ? 25 : $(this).height()/8)) +'px 0; font-size:'+($(this).height()>=150 ? 35 : $(this).height()/6)+'px;  height:'+($(this).height()>=150 ? 50 : $(this).height()/4)+'px" class="image_roll_text">'+text+'</div></div>');
	})
